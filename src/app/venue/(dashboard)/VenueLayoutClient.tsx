...
type NavKey = 'dashboard' | 'staff' | 'payouts' | 'settings' | 'profile';
...
type LocalizedNavItem = BaseNavItem & {
  labelKey: NavKey;
  label?: never;
};

type FixedNavItem = BaseNavItem & {
  label: string;
  labelKey?: never;
};

type NavItem = LocalizedNavItem | FixedNavItem;

export function VenueLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    ...
    { href: '/venue/staff', labelKey: 'staff', icon: Users },
    {
      href: '/venue/qr-codes',
      label: 'QR - codes',
      icon: QrCode,
